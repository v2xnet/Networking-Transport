...
#define tq_data(ptr, type, member) (type*)((char*)ptr - ((size_t) & ((type*)0)->member))
...
int add_task_abs(const struct timespec* expires, struct tq_elem* tqi, void (*task)(struct tq_elem*));

/* Add task to be triggered after expires_in */
static inline int add_task_rel(const struct timespec* expires_in, struct tq_elem* tqi, void (*task)(struct tq_elem*))
{
